...
import { BrowserFallbackClient, BrowserFallbackRelayUnavailableError, } from "./browser_fallback.js";
...
export class HttpClient {
    constructor(opts) {
        ...
        this.userAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/141.0.0.0 Safari/537.36 Edg/141.0.0.0";
        this.cache = new Map();
        this.cookies = new Map(); // Store cookies across requests
        this.lastUrl = null; // Track last URL for Referer header
        this.cloudscraperFailed = false; // Track if cloudscraper has failed
    ...
    headers() {
        ...
        if (this.opts.auth.type === "api_key") {
            ...
            if (this.opts.auth.username)
                h["Api-Username"] = this.opts.auth.username;
        }
        else if (this.opts.auth.type === "user_api_key") {
            h["User-Api-Key"] = this.opts.auth.key;
            if (this.opts.auth.client_id)
                h["User-Api-Client-Id"] = this.opts.auth.client_id;
        }
        return h;
    }
    ...
    async getCached(path, ttlMs, { signal } = {}) {
    ...
    isLoginRequired(finalUrl, bodyText) {
    ...
    async tryBrowserFallback(method, url, headers, body) {
    ...
    async requestViaBypass(method, url, headers, body) {
...
async function withRetries(fn, logger, url, method, retries = 3) {
    let attempt = 0;
    let delay = 250;
