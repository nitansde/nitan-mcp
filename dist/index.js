...
import { getDefaultBrowserFallbackEnabled, getDefaultBrowserFallbackProvider, resolveBrowserFallbackEnabled, resolveBrowserFallbackProvider, } from "./http/browser_fallback_defaults.js";
...
async function loadProfile(path) {
    ...
    if (!parsed.success)
        throw new Error(`Invalid profile JSON: ${parsed.error.message}`);
    return parsed.data;
}
function getDefaultPythonPath() {
    const cwdVenvPython = process.platform === "win32" ? ".venv\\Scripts\\python.exe" : ".venv/bin/python";
    if (existsSync(cwdVenvPython))
        return cwdVenvPython;
    const packageVenvPythonUrl = process.platform === "win32"
...
function mergeConfig(profile, flags) {
    // Handle simple username/password flags by creating auth_pairs entry
    let authPairs = flags.auth_pairs ?? profile.auth_pairs;
    // If username/password are provided via flags or environment variables, create an auth_pairs entry
    const username = flags.username ?? process.env.NITAN_USERNAME;
    const password = flags.password ?? process.env.NITAN_PASSWORD;
    const site = flags.site ?? profile.site ?? "https://www.uscardforum.com/";
    if (username && password && site) {
        ...
        const secondFactor = (flags.second_factor_token ?? flags["second-factor-token"]) ?? process.env.DISCOURSE_2FA_TOKEN;
        if (secondFactor) {
        ...
        if (!authPairs) {
            ...
            const existingIndex = authPairs.findIndex((entry) => entry.site === site);
            if (existingIndex >= 0) {
    ...
    const merged = {
        ...
        read_only: (flags.read_only ?? flags["read-only"]) ?? profile.read_only ?? true,
        allow_writes: (flags.allow_writes ?? flags["allow-writes"]) ?? profile.allow_writes ?? false,
        timeout_ms: (flags.timeout_ms ?? flags["timeout-ms"]) ?? profile.timeout_ms ?? DEFAULT_TIMEOUT_MS,
        concurrency: flags.concurrency ?? profile.concurrency ?? 4,
        cache_dir: (flags.cache_dir ?? flags["cache-dir"]) ?? profile.cache_dir,
        log_level: ((flags.log_level ?? flags["log-level"]) ?? profile.log_level ?? "info"),
        tools_mode: ((flags.tools_mode ?? flags["tools-mode"]) ?? profile.tools_mode ?? "auto"),
        site: site ?? "https://www.uscardforum.com/",
        default_search: ((flags.default_search ?? flags["default-search"]) ?? profile.default_search),
        max_read_length: ((flags.max_read_length ?? flags["max-read-length"]) ?? profile.max_read_length ?? 50000),
        transport: (flags.transport ?? profile.transport ?? "stdio"),
        port: (flags.port ?? profile.port ?? 3000),
        http_allow_reuse: ((flags.http_allow_reuse ?? flags["http-allow-reuse"]) ?? profile.http_allow_reuse ?? false),
        use_cloudscraper: ((flags.use_cloudscraper ?? flags["use-cloudscraper"]) ?? profile.use_cloudscraper),
        bypass_method: ((flags.bypass_method ?? flags["bypass-method"]) ?? profile.bypass_method ?? "both"),
        python_path: ((flags.python_path ?? flags["python-path"]) ?? profile.python_path ?? getDefaultPythonPath()),
        browser_fallback_enabled: ((flags.browser_fallback_enabled ?? flags["browser-fallback-enabled"]) ?? profile.browser_fallback_enabled ?? getDefaultBrowserFallbackEnabled()),
        browser_fallback_provider: resolveBrowserFallbackProvider(((flags.browser_fallback_provider ?? flags["browser-fallback-provider"]) ?? profile.browser_fallback_provider)),
        browser_fallback_timeout_ms: ((flags.browser_fallback_timeout_ms ?? flags["browser-fallback-timeout-ms"]) ?? profile.browser_fallback_timeout_ms ?? 45000),
        interactive_login_enabled: ((flags.interactive_login_enabled ?? flags["interactive-login-enabled"]) ?? profile.interactive_login_enabled ?? true),
        login_profile_name: ((flags.login_profile_name ?? flags["login-profile-name"]) ?? profile.login_profile_name),
        login_wait_timeout_ms: ((flags.login_wait_timeout_ms ?? flags["login-wait-timeout-ms"]) ?? profile.login_wait_timeout_ms ?? 180000),
        login_check_url: ((flags.login_check_url ?? flags["login-check-url"]) ?? profile.login_check_url),
        skip_site_validation: ((flags.skip_site_validation ?? flags["skip-site-validation"]) ?? profile.skip_site_validation ?? false),
    };
    const result = ProfileSchema.safeParse(merged);
    if (!result.success)
        throw new Error(`Invalid configuration: ${JSON.stringify(result.error.issues, null, 2)}`);
    return result.data;
}
function buildAuth(_config) {
 ...
 */
async function checkPythonDepsRuntime(logger, pythonPath) {
...
async function probePythonDeps(pythonPath) {
    return new Promise((resolve) => {
        const checkScript = `
import sys
try:
...
function platformInstallGuide(opts) {
...
async function runDoctor() {
    ...
    if (!pyProbe.ok)
        console.log(`  reason: ${pyProbe.reason}`);
    if (isMac) {
    ...
    if (!pyProbe.ok || (isMac && (!chromeExists || needsPlaywrightFix))) {
        ...
        for (const line of platformInstallGuide({ includePlaywrightFix: needsPlaywrightFix }))
            console.log(`  ${line}`);
    }
}
async function main() {
    ...
    const profilePath = argv.profile ?? undefined;
    ...
    let authOverrides = undefined;
    if (Array.isArray(config.auth_pairs)) {
        authOverrides = config.auth_pairs;
    }
    else if (typeof config.auth_pairs === "string") {
        try {
            const parsed = JSON.parse(config.auth_pairs);
            if (Array.isArray(parsed))
                authOverrides = parsed;
        }
        catch {
            // ignore
        }
    ...
    if (config.site) {
        try {
            ...
            if (!config.skip_site_validation) {
                const about = (await client.get(`/about.json`));
