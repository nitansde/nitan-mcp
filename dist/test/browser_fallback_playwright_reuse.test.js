...
function overridePlatform(platform) {
...
test("auto-login is attempted with NITAN env credentials and skipped when env is missing", async () => {
    ...
    try {
        ...
        if (originalUsername === undefined)
            ...
            process.env.NITAN_USERNAME = originalUsername;
        if (originalPassword === undefined)
            delete process.env.NITAN_PASSWORD;
        else
            process.env.NITAN_PASSWORD = originalPassword;
        restorePlatform();
        rmSync(homeDir, { recursive: true, force: true });
    }
});
test("recovers from singleton lock by terminating locked profile process and relaunching", async () => {
    ...
    client.terminateLockedProfileChromeProcesses = async () => {
        terminateCalls += 1;
        return 1;
    };
    try {
...
//# sourceMappingURL=browser_fallback_playwright_reuse.test.js.map