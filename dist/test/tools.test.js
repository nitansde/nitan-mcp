...
function expectSearchOutput(text) {
...
test('registers write-enabled tools when allowWrites=true', async () => {
    ...
    const siteState = createSiteState();
    const tools = {};
    const fakeServer = { registerTool(name, _meta, handler) { tools[name] = { handler }; } };
    await registerAllTools(fakeServer, siteState, logger, { allowWrites: true, toolsMode: 'discourse_api_only' });
...
test('tethered mode hides select_site and allows search without selection', async () => {
    ...
    try {
        ...
        await registerAllTools(fakeServer, siteState, logger, { allowWrites: false, toolsMode: 'discourse_api_only', hideSelectSite: true });
...
test('default-search prefix is applied to queries', async () => {
    ...
    try {
        ...
        await client.get('/about.json');
        siteState.selectSite(base);
        await registerAllTools(fakeServer, siteState, logger, { allowWrites: false, toolsMode: 'discourse_api_only', defaultSearchPrefix: 'tag:ai order:latest-post' });
