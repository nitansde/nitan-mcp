import { z } from "zod";
let lastPostAt = 0;
export const registerCreatePost = (server, ctx, opts) => {
    if (!opts.allowWrites)
    ...
    server.registerTool("discourse_create_post", {
        title: "Create Post",
        description: "Create a post in a topic.",
        inputSchema: schema.shape,
    }, async ({ topic_id, raw }, _extra) => {
        ...
        try {
            ...
            const data = (await client.post(`/posts.json`, { topic_id, raw }));
