import { z } from "zod";
export const registerReadPost = (server, ctx) => {
    const schema = z.object({
        post_id: z.number().int().positive(),
    });
    server.registerTool("discourse_read_post", {
        title: "Read Post",
        ...
        try {
            ...
            const data = (await client.getCached(`/posts/${post_id}.json?include_raw=true`, 10000));
            const username = data?.username || data?.user_id || "user";
            const created = data?.created_at || "";
            const raw = data?.raw || data?.cooked || "";
...
//# sourceMappingURL=read_post.js.map