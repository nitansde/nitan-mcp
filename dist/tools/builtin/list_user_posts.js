...
export const registerListUserPosts = (server, ctx) => {
    const schema = z.object({
        username: z.string().min(1),
        page: z.number().int().min(0).optional(),
    });
    server.registerTool("discourse_get_user_activity", {
    ...
    }, async ({ username, page }, _extra) => {
        try {
            const { base, client } = ctx.siteState.ensureSelectedSite();
            const offset = (page || 0) * 30;
            // The filter parameter 4,5 corresponds to posts and replies
            const data = (await client.get(`/user_actions.json?offset=${offset}&username=${encodeURIComponent(username)}&filter=4,5`));
            ...
            const posts = userActions.map((action) => {
...
//# sourceMappingURL=list_user_posts.js.map