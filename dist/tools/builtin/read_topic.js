...
export const registerReadTopic = (server, ctx) => {
    ...
    }, async ({ topic_id, post_limit = 90, start_post_number = 1, username_filter }, _extra) => {
        try {
            ...
            if (username_filter) {
                ...
                const metaData = (await client.get(metaUrl));
                ...
                while (fetchedPosts.length < post_limit) {
                    ...
                    const rawText = (await client.get(rawUrl));
            ...
            if (username_filter)
                lines.push(`Filtered by user: @${username_filter}`);
            lines.push("");
...
//# sourceMappingURL=read_topic.js.map