...
export const registerListNotifications = (server, ctx) => {
    const schema = z
        .object({
        limit: z
            .number()
        ...
        try {
            ...
            for (const notif of notifications) {
                if (notif.notification_type === 2 && notif.topic_id && notif.post_number) {
                    try {
                        const rawContent = (await client.get(`/raw/${notif.topic_id}/${notif.post_number}`));
...
//# sourceMappingURL=list_notifications.js.map