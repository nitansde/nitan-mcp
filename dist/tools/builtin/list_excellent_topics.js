...
export const registerListExcellentTopics = (server, ctx, _opts) => {
    const schema = z
        .object({
        limit: z
            .number()
        ...
        .strict();
    server.registerTool("discourse_list_excellent_topics", {
        title: "List Excellent Topics",
        description: "Get recent excellent topics from the forum. An excellent topic is a topic with over 50 likes. Returns the most recent topics that earned the '精彩的话题' (Excellent Topic) badge.",
        inputSchema: schema.shape,
    }, async ({ limit = 20 }, _extra) => {
        try {
            const { base, client } = ctx.siteState.ensureSelectedSite();
            // Badge ID 20 is for "精彩的话题" (Excellent Topic - 50 likes)
            const data = (await client.get(`/user_badges.json?badge_id=20`));
            ...
            const results = recentBadges.map((badge) => {
            ...
            ctx.logger.error(`Failed to fetch excellent topics: ${e?.message || String(e)}`);
            return {
                content: [
                    {
                        type: "text",
...
//# sourceMappingURL=list_excellent_topics.js.map