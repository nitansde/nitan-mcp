...
export const registerListFunnyTopics = (server, ctx, _opts) => {
    const schema = z
        .object({
        limit: z
            .number()
        ...
        .strict();
    server.registerTool("discourse_list_funny_topics", {
        title: "List Funny Topics",
        description: "Get recent funny topics from the forum. A funny topic is a topic that earned the '难绷的话题' (Funny Topic) badge.",
        inputSchema: schema.shape,
    }, async ({ limit = 20 }, _extra) => {
        try {
            const { base, client } = ctx.siteState.ensureSelectedSite();
            // Badge ID 115 is for "难绷的话题" (Funny Topic)
            const data = (await client.get(`/user_badges.json?badge_id=115`));
            ...
            ctx.logger.error(`Failed to fetch funny topics: ${e?.message || String(e)}`);
            return {
                content: [
                    {
                        type: "text",
...
//# sourceMappingURL=list_funny_topics.js.map