...
export const registerFilterTopics = (server, ctx) => {
    const schema = z
        .object({
        categories: z
            .array(z.string())
        ...
        filter: z
            .string()
            .optional()
            .describe("Additional filter query (optional), e.g. 'status:open created-after:30 order:activity'. If categories are provided, they will be added to this filter."),
        page: z
            .number()
            .int()
            .min(0)
        ...
        .strict();
    const description = "Filter topics by categories and other criteria. You can specify categories by their natural language names (e.g., '信用卡', '旅行', '败家'). " +
    ...
    server.registerTool("discourse_filter_topics", {
        title: "Filter Topics",
        description,
        inputSchema: schema.shape,
    }, async ({ categories, filter = "", page = 0, per_page }, _extra) => {
        try {
            ...
            const data = (await client.get(`/search.json?${params.toString()}`));
            ...
            const moreUrl = undefined; // Search API doesn't provide this
            const lines = [];
...
//# sourceMappingURL=filter_topics.js.map